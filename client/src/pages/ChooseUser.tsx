...
// Interface for ChooseUser props (optional)
interface ChooseUserProps {
  visitor: "normal" | "guest";
}

const ChooseUser: React.FC<ChooseUserProps> = ({ visitor }) => {
  ...
  const navigateHandler = (user: string) => {
    if (user === "Admin") {
      if (visitor === "guest") {
        ...
        setLoader(true);
        dispatch<any>(loginUser(fields, user));
      } else {
      ...
      if (visitor === "guest") {
        const rollNum = "1";
        const studentName = "Adekunle Osas";
